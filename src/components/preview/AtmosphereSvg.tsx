...
import { LIVECARD_COVES, LIVECARD_COVE_W, LIVECARD_COVE_GAP, LIVECARD_SQUARE_SIZE, LIVECARD_TOTAL_W, LIVECARD_COVE_H } from '../../constants/livecard'
...
export function LivecardSquarePreview({ scale, idPrefix }: { scale: number; idPrefix: string }) {
  ...
  const radius = computeRadius(SQ, SQ)
  const blur = computeDefaultBlur(SQ, SQ)
  const pad = computeFilterPadding(blur, radius)
  const frequency = computeNoiseFrequency(SQ, SQ, noiseScale)
  ...
  return (
    <svg width={displaySize} height={displaySize} viewBox={`0 0 ${SQ} ${SQ}`} style={{ display: 'block' }}>
      <defs>
        <clipPath id={clipId}>
          <rect width={SQ} height={SQ} />
        </clipPath>
        <filter id={blurId} {...filterRegion}>
      ...
      <g clipPath={`url(#${clipId})`}>
        ...
        {squareCircles.map((c) => (
          <circle key={c.role} cx={c.x} cy={c.y} r={radius} fill={c.color} filter={`url(#${blurId})`} />
        ))}
        {noiseIntensity > 0 && (
          <rect width={SQ} height={SQ} fill="transparent" filter={`url(#${noiseId})`} opacity={noiseIntensity} style={{ mixBlendMode: 'soft-light' }} />
        )}
        {showLogo && (
          <g transform={`translate(${logoX}, ${logoY}) scale(${logoScl})`}>
...
export function AtmosphereSvg({ scale, displayWidth, displayHeight, interactive = true, gapColor = '#1d0029' }: Props) {
  ...
  return (
    ...
    >
      ...
      <g clipPath="url(#canvas-clip)">
        ...
        {triptych && (
          <>
            <rect x={TRIPTYCH_PANEL_W} y={0} width={TRIPTYCH_GAP} height={H} fill={gapColor} />
            <rect x={TRIPTYCH_PANEL_W * 2 + TRIPTYCH_GAP} y={0} width={TRIPTYCH_GAP} height={H} fill={gapColor} />
          </>
        )}

        {livecard && LIVECARD_COVES.slice(0, -1).map((cove) => (
          <rect key={cove.label} x={cove.x + LIVECARD_COVE_W} y={0} width={LIVECARD_COVE_GAP} height={H} fill={gapColor} />
        ))}
      </g>

      {interactive && (showGuides || draggingRole !== null) && <GuideOverlay radius={radius} activeRole={showGuides ? null : draggingRole} />}
      {interactive && circles.map((c) => (
