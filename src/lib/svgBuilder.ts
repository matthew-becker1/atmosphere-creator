...
import { LIVECARD_SQUARE_SIZE } from '../constants/livecard'
...
export function buildLivecardSquareSvg(state: AppState): string {
  const SQ = LIVECARD_SQUARE_SIZE
  const squareCircles = state.circles.map((c) => ({
    ...c,
    x: c.anchorX * SQ,
    y: c.anchorY * SQ,
  }))
  const squareState: AppState = {
    ...state,
    width: SQ,
    height: SQ,
    triptych: false,
    livecard: false,
    circles: squareCircles as AppState['circles'],
  }
  return buildSvgString(squareState)
}
